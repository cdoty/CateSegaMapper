...
ap_getbit:
  push bc
    ld bc,(apLib_bits)
    rrc c
    jr nc,ap_getbit_branch
    ld b,(hl)
    inc hl
ap_getbit_branch:  ld a,c
    and b
    ld (apLib_bits),bc
  pop bc
  ret
...
ap_getgamma:
  ld bc,1
ap_getgamma_branch:call ap_getbitbc
  call ap_getbit
  jr nz,ap_getgamma_branch
  ret
...
_ap_not_LWM:
  ...
  push bc
    ...
    push de
      ...
      jr nc,_ap_not_LWM_1
      inc bc
_ap_not_LWM_1:    ld hl,-1280
      add hl,de
      jr nc,_ap_not_LWM_2
      inc bc
_ap_not_LWM_2:    ld hl,-128
      add hl,de
      jr c,_ap_not_LWM_3
      inc bc
      inc bc
_ap_not_LWM_3:    ;bc = len, de = offs, hl=junk
    pop hl
    push hl
      or a
      sbc hl,de
...
_apbranch3:
  ...
  ret z
  ld b,2
  jr nc,_apbranch3_branch
  inc b
_apbranch3_branch:;LWM = 1
  ld a,1
  ld (apLib_LWM),a
...
ap_VRAMToDE_write:
  push af
    ...
    ld a,d
    or $40
ap_VRAMToDE_write_branch:  out ($bf),a
  pop af
  ret
ap_VRAMToHL_read:
  push af
    ...
    out ($bf),a
    ld a,h
    jr ap_VRAMToDE_write_branch ; space optimisation
...
ap_VRAM_ldir_dest_to_dest:
  ...
  push af
ap_VRAM_ldir_dest_to_dest_branch:  call ap_VRAMToHL_read
    in a,($be)
    call ap_VRAMToDE_write
    out ($be),a
    dec bc
    ...
    ld a,b
    or c
    jr nz,ap_VRAM_ldir_dest_to_dest_branch
  pop af
  ret
...
_vram_ap_not_LWM:
  ...
  push bc
    ...
    push de
      ...
      ld hl,-32000
      add hl,de
      jr nc,_vram_ap_not_LWM_1
      inc bc
_vram_ap_not_LWM_1:    ld hl,-1280
      add hl,de
      jr nc,_vram_ap_not_LWM_2
      inc bc
_vram_ap_not_LWM_2:    ld hl,-128
      add hl,de
      jr c,_vram_ap_not_LWM_3
      inc bc
      inc bc
_vram_ap_not_LWM_3:    ;bc = len, de = offs, hl=junk
    pop hl
    push hl
      or a
      sbc hl,de
...
_vram_apbranch3:
  ...
  ret z
  ld b,2
  jr nc,_vram_apbranch3_branch
  inc b
_vram_apbranch3_branch:;LWM = 1
  ld a,1
  ld (apLib_LWM),a
...
apLib_bits:
	ds	1
apLib_byte:	; not directly referenced, assumed to come after bits
	ds	1
apLib_LWM:
	ds	1
apLib_R0:
